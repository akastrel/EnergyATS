...
- смена `primary_generator` влияет на следующую новую сессию и не меняет
  генератор уже начатой сессии;
...
Имена и модели больше не хранятся в конфигурации Energy ATS и не зашиты в
...
Перед первым запуском или обновлением рекомендуется:
...
человеко-читаемый статус, а атрибуты включают, в частности:

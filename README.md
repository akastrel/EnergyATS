...
Версия **0.3.1** приводит реализацию этой архитектуры в точное соответствие с
проверенными сценариями из `docs/REQUIREMENTS_RU.md`. Приложение остаётся одним
процессом:
...
- автоматический fallback на второй генератор в 0.3.1 отключён;
...
Для 0.3.1 сначала требуется прошить Generator Controller 0.3.1 с новыми
однозначными кнопками `choke_to_cold_start/choke_to_run`. Обновляться следует при
...
Текущий статус: **0.3.1**, experimental.

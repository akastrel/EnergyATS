...
Небольшие уточнения пользовательского состояния по результатам физических тестов M4-A и M5 без изменения силовой policy EnergyATS.
...
- M4-A записан как PASS: `grid_input_ready` отсутствовал около 34,1 с при рабочей задержке 60 с, генераторы не запускались. M4-B остаётся pending.

...
- Во время базового `grid_failure_delay` статус называется «Проверка отсутствия сети» вместо прежнего «Ожидание запуска генератора».
- После перехода UPS Run в `waiting_on_ups` App log и `sensor.energy_ats_status` показывают «Питание от UPS»; внутренний Supervisor phase остаётся `grid_failure_delay` для совместимости FSM.
- Сообщение о базовой задержке больше не обещает обязательный запуск резервного питания после её окончания: дальнейшее решение может принять Delayed Start.
- Сообщение Delayed Start понятнее объясняет, что АВР продолжает работу от батарей UPS до возвращения сети или снижения заряда до настроенного порога.
- Обе части M4 записаны как PASS: краткая потеря Grid не запустила генератор, а её краткое возвращение не начало преждевременный возврат дома на сеть.
- После фактического возврата питания дома на Grid подтверждение «Подтверждено: дом питается от основной сети» публикуется в основном пользовательском журнале; промежуточные подтверждения контакторов остаются только в полном App log.
- M5 записан как PASS с некритичным замечанием к отсутствовавшему в основном журнале подтверждению завершённого возврата на Grid.
- Пользовательская настройка максимальной работы от UPS задаётся в часах: `generator_max_start_delay_hours = 6`; старое значение `generator_max_start_delay` в секундах автоматически преобразуется при обновлении.
- Названия и пояснения всех настроек UPS Run изложены по-русски, без внутренних терминов реализации.

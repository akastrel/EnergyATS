# Установка и обновление Energy ATS 0.3.10

Energy ATS 0.3.10 использует описанный ниже контракт Generator Controller 0.3.1.
Ручные команды передаются непосредственно
...
После обновления списка Apps установить версию 0.3.10.
...
предложенную конфигурацию 0.3.10 и убедиться, что старых полей больше нет.
...
Текущая версия использует внутренний файл:
...
Если Energy ATS впервые запущен при уже работающем генераторе без своего журнала,
...
Если связь потеряна посередине физической транзакции, App не продолжает старую
транзакцию автоматически. После восстановления связи:

1. осмотреть фактическое состояние оборудования;
2. снять Emergency Stop, если он был включён;
3. выполнить команду `reset`.

При однозначных обратных связях App сам выполняет ограниченное восстановление:
сначала снимает генераторную шину, затем подключает Grid path, после чего
выполняет cooldown и останавливает только генератор прерванной управляемой
сессии. Внешний генератор App не останавливает. При одновременном подключении
источников, неизвестных данных или другой неоднозначной топологии аппаратные
команды не выполняются, а `RECOVERY_REQUIRED` сохраняется.

...
### Эксплуатационная критичность сущностей

Для эксплуатации полезно различать четыре класса HA-сущностей:

- **CORE / safety-critical** — физические сигналы и исполнительные устройства, без которых АВР не может доказать безопасное состояние силовой схемы или выполнить управляемое переключение. `unknown/unavailable` во время операции блокирует соответствующие действия и в некоторых сценариях приводит к `RECOVERY_REQUIRED`.
- **CORE / required** — обязательные для штатной конфигурации и принятия основных решений сущности, но не являющиеся непосредственным физическим feedback силовой коммутации.
- **SOFT** — входы дополнительных функций. Их отказ ограничивает UPS Run, Load Manager, Scheduled Exercise или заставляет использовать консервативный fallback, но сам по себе не должен переводить основной АВР в Recovery.
- **OUTPUT / diagnostic** — публикации АВР для интерфейса, журналирования и диагностики. Они не используются как управляющий feedback.

К **CORE / safety-critical** относятся прежде всего `grid_input_ready`, feedback основных контакторов, `grid_power`, `use_generator_as_power_source`, RUNNING/REMOTE обоих генераторов и `generators_emergency_stop`. Команды choke относятся к физическому управлению запуском соответствующего генератора. К **SOFT** относятся UPS telemetry, Load Manager meter/G1/G2/power limits, presence Scheduled Exercise и температура гаража. `sensor.energy_ats_status` — **OUTPUT / diagnostic**.
